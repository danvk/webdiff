...
test('span spreads across lines', () => {
  ...
  expect(['<span class=jsdoc>foo</span>',
                    '<span class=jsdoc>bar</span>']).toEqual(
                    lines);
});
...
test('nested spans', () => {
  ...
  var lines = distributeSpans(html);
  expect(['<span class=jsdoc>foo</span>',
                    '<span class=jsdoc><span>bar</span> blah</span>']).toEqual(
                    lines);
});
...
test('multiple multi-line spans', () => {
  ...
  var lines = distributeSpans(html);
  expect(lines).toEqual(['<span class=jsdoc>foo<span>blah</span></span>',
